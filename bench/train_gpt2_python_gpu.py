#!/usr/bin/env python
"""Benchmark: GPT-2 training loop with polygrad (CUDA GPU)."""
...
print(f"{'='*70}")
print(f"GPT-2 Training Benchmark (polygrad CUDA GPU)")
print(f"{'='*70}\n")
...
for cfg in CONFIGS:
    ...
    print(f"  Parameters: {len([p for p in get_parameters(GPT2(n_layers=cfg['n_layers'], n_heads=cfg['n_heads'], dim=cfg['dim'], vocab_size=cfg['vocab_size'], max_seq_len=cfg['seq_len']*2))])} tensors, {n_params:,} weights")
...
print(f"{'='*70}")
print(f"polygrad CUDA GPU, segment-wise backward.")
print(f"{'='*70}")
