...
static int build_hip_binds(PolyBufferBinding *out, PolyUOp **bufs, float **host_ptrs, int n) {
  for (int i = 0; i < n; i++) {
    if (!bufs[i] || bufs[i]->n_src != 1 || !bufs[i]->src[0] ||
        bufs[i]->src[0]->op != POLY_OP_CONST || bufs[i]->src[0]->arg.kind != POLY_ARG_INT)
      return -1;
    size_t nbytes = (size_t)bufs[i]->src[0]->arg.i * poly_dtype_itemsize(bufs[i]->dtype);
