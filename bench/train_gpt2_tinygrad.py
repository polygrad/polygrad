#!/usr/bin/env python
"""Benchmark: GPT-2 training loop with tinygrad (CPU) for comparison.

Mirrors train_gpt2_python.py configs exactly. Uses tinygrad's built-in
GPT-2 implementation with the same hyperparameters.
"""
...
class Embedding:
    ...
    def __call__(self, idx):
        # one-hot matmul embedding (same as polygrad)
...
print(f"{'='*70}")
print(f"GPT-2 Training Benchmark (tinygrad CPU/CLANG)")
print(f"{'='*70}\n")
...
print(f"{'='*70}")
print(f"tinygrad CPU backend (ref commit c2be31e), single-pass backward.")
print(f"{'='*70}")
