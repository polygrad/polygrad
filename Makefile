...
LOADER_SRC = src/loaders/decoded.c src/loaders/import_error.c src/loaders/bind.c src/loaders/hf_decode.c src/loaders/gguf_decode.c src/loaders/gguf_loader.c
SRC += src/uop/key.c
FILC_SRC += src/uop/key.c
CODEC_SRC = vendor/cjson/cJSON.c src/safetensors.c src/wlrn.c src/ir.c src/bundle.c src/model.c src/tokenizer.c src/models/compose.c src/models/layers.c src/models/mlp.c src/models/tabm.c src/models/nam.c src/models/registry.c src/models/gpt2.c src/models/distilgpt2.c src/models/qwen3.c src/models/llama.c src/models/hf_loader.c $(LOADER_SRC)
...
CODEC_SRC += src/models/vision.c src/models/clip.c src/models/vit.c src/models/dinov2.c src/models/dinov3.c
ANALYZE_FLAGS ?=
...
test-autograd-wasm: build/test_schedule_runtime.js
	$(SAN_RUN) $(NODE) $< --require-no-skips substitute
...
.PHONY: test-vision
test-vision: build/polygrad_test build/libpolygrad.so
	$(SAN_RUN) ./build/polygrad_test --require-no-skips hf.vision_config_factories_require_weights
