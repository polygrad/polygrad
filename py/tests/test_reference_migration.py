...
def gate(tmp_path):
    ...
    put(tmp_path, "test/tensor_graph_cases.py", "CASES = {'owner_case': ('tensor', None)}\n")
...
def test_filtered_graph_report_cannot_certify_full_corpus(gate):
    put(gate[0], "test/tensor_graph_cases.py", "CASES = {'owner_case': ('tensor', None), 'omitted': ('tensor', None)}\n")
    inputs = migration.source_manifest(gate[0])
    gate[3]["source_inputs"] = inputs
    for check in gate[3]["checks"].values():
        check["source_sha256"] = migration.manifest_hash(inputs)
