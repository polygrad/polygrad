...
from polygrad.helpers import DEBUG, ContextVar, GlobalCounters, getenv
    ...
    def write_events():
        ...
        for event in _events:
            if isinstance(event, BenchEvent):
                for values in _events[event].values():
                    if len(values) > 0:
                        have_events = True
            elif len(_events[event]) > 0:
                have_events = True
        if not have_events:
            return
            ...
            if isinstance(event, BenchEvent):
                for event_type, values in _events[event].items():
                    for i, value in enumerate(values):
                        points.append(_create_point(
                            run_id, i, attempt, ref, commit,
                            f"{event.value}_{event_type}", value, run,
                        ))
            else:
                for i, value in enumerate(_events[event]):
                    points.append(_create_point(
                        run_id, i, attempt, ref, commit, event.value, value, run,
                    ))
