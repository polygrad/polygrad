...
TEST(mlp, create_simple) {
  ...
  ASSERT_INT_EQ(poly_model_param_count(inst), 4);

  /* Check param names */
  ASSERT_STR_EQ(poly_model_param_name(inst, 0), "layers.0.weight");
  ASSERT_STR_EQ(poly_model_param_name(inst, 1), "layers.0.bias");
  ASSERT_STR_EQ(poly_model_param_name(inst, 2), "layers.1.weight");
  ASSERT_STR_EQ(poly_model_param_name(inst, 3), "layers.1.bias");
...
TEST(mlp, kaiming_bounds) {
  ...
  float *b = poly_model_param_data(inst, 1, &numel);
  for (int64_t i = 0; i < numel; i++) {
...
TEST(mlp, forward_and_train_replay_stats_plateau) {
  ...
  ASSERT_INT_EQ(poly_model_forward(inst, forward_io, 1), 0);
  ...
  ASSERT_INT_EQ(
      poly_model_set_optimizer(inst, POLY_OPTIM_ADAM, 0.001f, 0.9f, 0.999f, 1e-8f, 0.0f), 0
  );
  float loss = 0.0f;
  ASSERT_INT_EQ(poly_model_train_step(inst, NULL, train_io, 2, &loss), 0);
