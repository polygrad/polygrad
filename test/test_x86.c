...
static PolyUOp *x86_lane(PolyCtx *ctx, PolyUOp *value, int lane) {
  ...
  return poly_uop2(
      ctx, POLY_OP_INDEX, value->dtype, value, idx, poly_arg_none()
  );
}
...
TEST_BACKEND(x86, pre_isel_eliminates_current_gated_load) {
  ...
  PolyUOp *zero = poly_uop1(
      ctx, POLY_OP_CAST, POLY_INT32,
      poly_uop0(ctx, POLY_OP_CONST, POLY_WEAKINT, poly_arg_int(0)), poly_arg_dtype(POLY_INT32)
  );
  PolyUOp *addr = poly_uop2(ctx, POLY_OP_INDEX, POLY_FLOAT32, src, zero, poly_arg_none());
  PolyUOp *one = poly_uop1(
      ctx, POLY_OP_CAST, POLY_INT32,
      poly_uop0(ctx, POLY_OP_CONST, POLY_WEAKINT, poly_arg_int(1)), poly_arg_dtype(POLY_INT32)
  );
  PolyUOp *two = poly_uop1(
      ctx, POLY_OP_CAST, POLY_INT32,
      poly_uop0(ctx, POLY_OP_CONST, POLY_WEAKINT, poly_arg_int(2)), poly_arg_dtype(POLY_INT32)
  );
  PolyUOp *gate_a = poly_uop2(
      ctx, POLY_OP_CMPEQ, POLY_BOOL, one, one, poly_arg_none()
  );
  PolyUOp *gate_b = poly_uop2(
      ctx, POLY_OP_CMPEQ, POLY_BOOL, two, two, poly_arg_none()
  );
  ...
  PolyUOp *store = poly_uop2(
      ...
      poly_uop2(ctx, POLY_OP_INDEX, POLY_FLOAT32, out, zero, poly_arg_none()), load,
      poly_arg_none()
  );
...
TEST_BACKEND(x86, isel_stack_lanes_use_current_vinsertps) {
  ...
  PolyUOp *src0[4] = {
      x86_lane(ctx, a, 0), x86_lane(ctx, b, 1),
      x86_lane(ctx, a, 2), x86_lane(ctx, b, 3),
  };
  PolyUOp *src1[4] = {
      x86_lane(ctx, a, 3), x86_lane(ctx, b, 2),
      x86_lane(ctx, c, 1), d,
  };
...
TEST_BACKEND(x86, isel_complex_address_scales_current_index) {
  ...
  PolyUOp *one = poly_uop1(
      ctx, POLY_OP_CAST, POLY_INT32, poly_const_int(ctx, 1), poly_arg_dtype(POLY_INT32)
  );
...
TEST_BACKEND(x86, to_program_allows_f16_after_x86_extra_legalization) {
  ...
  ASSERT_INT_EQ(linear_schedule->n_src, 1);
  PolyUOp *program =
      poly_test_linear_call_body(poly_compile_linear(ctx, linear_schedule, -1), 0);
  ASSERT_NOT_NULL(program);
  ASSERT_INT_EQ(program->n_src, 4);
  ASSERT_INT_EQ(program->src[2]->op, POLY_OP_SOURCE);
  ASSERT_INT_EQ(program->src[3]->op, POLY_OP_BINARY);
...
TEST_BACKEND(x86, canonical_reg_buffer_extent_survives_isel) {
  ...
  PolyUOp *size = poly_uop1(
      ctx, POLY_OP_CAST, POLY_INT32,
      poly_uop0(ctx, POLY_OP_CONST, POLY_WEAKINT, poly_arg_int(4)),
      poly_arg_dtype(POLY_INT32)
  );
  ...
  for (int i = 0; i < n_lin; i++) {
    ...
    if (!u || u->op != POLY_OP_INS || u->arg.kind != POLY_ARG_INT ||
        u->arg.i != POLY_X86_SUBi || u->n_src != 1 || !u->src[0] ||
        u->src[0]->op != POLY_OP_CAST || u->src[0]->n_src != 1 ||
        u->src[0]->tag_arg.kind != POLY_ARG_BOOL || !u->src[0]->tag_arg.b ||
        !u->src[0]->src[0] || u->src[0]->src[0]->op != POLY_OP_CONST ||
        u->src[0]->src[0]->arg.kind != POLY_ARG_INT)
      continue;
