...
static int migrate_to_device(PolyCtx *ctx, PolyUOp *tensor_sink,
                             PolyBufferBinding *bindings, int n_bindings,
                             PolyDeviceId device,
                             PolyBufferBinding **out_dev, int *out_total) {
  ...
  for (int i = 0; i < n_bindings; i++) {
    ...
    if (!dptr) {
      for (int j = 0; j < i; j++) alloc->free(dev[j].handle.ptr, alloc->dev_ctx);
      free(dev);
      return -1;
    }
...
static void unmigrate_from_device(PolyBufferBinding *host_bindings, int n_user,
                                  PolyBufferBinding *dev, int n_total,
                                  PolyDeviceId device, bool readback) {
  const PolyBackendDesc *be = poly_backend_get(device);
  if (!be) { free(dev); return; }
  const PolyAllocator *alloc = be->get_allocator();

  if (readback) {
    for (int i = 0; i < n_user; i++) {
      if (!host_bindings[i].handle.ptr) continue;
      alloc->copy_out(host_bindings[i].handle.ptr, dev[i].handle.ptr,
                      dev[i].handle.nbytes, alloc->dev_ctx);
    }
  }
...
int poly_realize_ex(PolyCtx *ctx, PolyUOp *tensor_sink,
  ...
  if (needs_device_migration(device, bindings, n_bindings)) {
    ...
    if (migrate_to_device(ctx, tensor_sink, bindings, n_bindings, device, &dev, &total) != 0)
      return -1;
    int ret = poly_realize_ex(ctx, tensor_sink, dev, total,
                              var_bindings, n_var_bindings);
    unmigrate_from_device(bindings, n_bindings, dev, total, device, ret == 0);
    return ret;
  }
...
PolyUOp *poly_dot_v2(PolyCtx *ctx, PolyUOp *x, PolyUOp *w) {
  ...
  x_ndim = uop_shape(ctx, x, x_shape);
  w_ndim = uop_shape(ctx, w, w_shape);
...
PolyUOp *poly_cross_entropy_v2(PolyCtx *ctx, PolyUOp *logits, PolyUOp *target, int axis) {
  ...
  l_ndim = uop_shape(ctx, logits, l_shape);
  t_ndim = uop_shape(ctx, target, t_shape);
...
PolyUOp *poly_linear_v2(PolyCtx *ctx, PolyUOp *x, PolyUOp *w, PolyUOp *bias) {
  ...
  if (bias) { b_ndim = uop_shape(ctx, bias, b_shape); }
  int64_t out_shape[POLY_MAX_DIMS]; int out_ndim;
...
PolyUOp *poly_gather_v2(PolyCtx *ctx, PolyUOp *table, PolyUOp *indices) {
  ...
  t_ndim = uop_shape(ctx, table, t_shape);
  i_ndim = uop_shape(ctx, indices, i_shape);
...
PolyUOp *poly_rmsnorm_v2(PolyCtx *ctx, PolyUOp *x, PolyUOp *weight, double eps) {
  ...
  if (weight) {
    ...
    w_ndim = uop_shape(ctx, weight, w_shape);
    if (w_ndim == 1) {
...
PolyUOp *poly_sdpa_v2(PolyCtx *ctx, PolyUOp *q, PolyUOp *k, PolyUOp *v,
  ...
  int64_t kt_shape[POLY_MAX_DIMS]; int kt_ndim;
  kt_ndim = uop_shape(ctx, k_t, kt_shape);
