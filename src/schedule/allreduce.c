...
static PolyUOp *handle_allreduce_naive(
    PolyCtx *ctx,
    PolyUOp *buf,
    PolyUOp *red
) {
  ...
  for (int i = 0; contiguous && target_device && i < source_device->arg.string_tuple.n; i++) {
    ...
    reduced = !reduced ? copy
                       : (copy ? poly_alu2(ctx, red->arg.allreduce.op, reduced, copy) : NULL);
  }
  return reduced;
}
...
PolyUOp *poly_create_allreduce_function(PolyCtx *ctx, PolyUOp *red) {
  ...
  PolyUOp *target_device = poly_uop_device_uop_cached(ctx, red, NULL);
  PolyUOp *output = target_device
                        ? poly_uop_new_buffer(
                              ctx, target_device, numel,
                              red->dtype,
  ...
  PolyUOp *param_red = src ? poly_uop1(
                                 ctx, POLY_OP_ALLREDUCE, red->dtype, src, red->arg)
                           : NULL;
  PolyUOp *reduced = param_red
                         ? handle_allreduce_naive(ctx, src, param_red)
                         : NULL;
