...
static PolyUOp *multi_clone(PolyCtx *ctx, PolyUOp *u, PolyUOp **src, int n_src) {
...
static bool multi_pm_is_device_range(PolyUOp *u) {
  return u && u->op == POLY_OP_RANGE && poly_range_axis_type(u->arg) == POLY_AXIS_DEVICE;
}
...
static PolyUOp *multi_pm_local_shrink(
  ...
  PolyUOp **values = n_device_ranges > 0 ? malloc((size_t)n_device_ranges * sizeof(*values)) : NULL;
  if (n_device_ranges > 0 && !values) return NULL;
  for (int i = 0; i < n_device_ranges; i++) {
    values[i] = poly_uop0(ctx, POLY_OP_CONST, device_ranges[i]->dtype, poly_arg_int(device_index));
    if (!values[i]) {
      free(values);
      return NULL;
    }
...
static PolyUOp *multi_pm_mstack_early_shrink(
  ...
  *failed = false;
  ...
  if (!multi_pm_collect_device_ranges(ctx, starts, sizes, &device_ranges, &n_device_ranges)) {
    *failed = true;
    return NULL;
  }
  ...
  for (int i = 0; i < mstack->n_src; i++) {
    ...
    PolyUOp *local_shrink =
        base ? multi_pm_local_shrink(ctx, base, starts, sizes, device_ranges, n_device_ranges, i)
             : NULL;
    if (!local_shrink) {
      *failed = true;
      break;
  ...
  PolyUOp *ret = *failed ? NULL : multi_clone(ctx, mstack, local, mstack->n_src);
  if (!ret) *failed = true;
  free(local);
  free(device_ranges);
  return ret;
...
static PolyUOp *multi_pm_restore_sharding(PolyCtx *ctx, PolyUOp *value, PolyUOp *template) {
...
static PolyUOp *multi_pm_shard_value(
  ...
  *failed = false;
  ...
  PolyUOp *local_dim = poly_uop2(ctx, POLY_OP_FLOORDIV, POLY_WEAKINT, dim, count, poly_arg_none());
  ...
  PolyUOp *start =
      sharding_range
          ? poly_uop2(ctx, POLY_OP_MUL, POLY_WEAKINT, sharding_range, local_dim, poly_arg_none())
          : NULL;
...
static PolyUOp *multi_pm_shard_subview(PolyCtx *ctx, PolyUOp *full, PolyUOp *multi, bool *failed);
...
static bool multi_pm_shard_idx(PolyCtx *ctx, PolyUOp *range, int device_index, int *out) {
  PolyUOp **device_ranges = NULL;
  int n_device_ranges = 0;
  if (!multi_pm_collect_device_ranges(ctx, range, range, &device_ranges, &n_device_ranges))
    return false;
  if (n_device_ranges == 0) {
    free(device_ranges);
    *out = 0;
  ...
  if (ok &&
      poly_uop_substitute_many(ctx, &root, 1, device_ranges, values, n_device_ranges, &rewritten) !=
          0)
    ok = false;
  rewritten = ok ? poly_graph_rewrite(ctx, rewritten, poly_symbolic()) : NULL;
  int64_t value = 0;
  if (!rewritten || poly_uop_const_i64(rewritten, &value) != 0 || value < 0 || value > INT_MAX)
    ok = false;
  free(values);
  free(device_ranges);
  if (ok) *out = (int)value;
...
static PolyUOp *multi_pm_copy_multi(
  ...
  *failed = false;
  ...
  if (!multi_pm_sharding(ctx, multi, axes, ranges, counts, &n_sharding) || !target_device ||
      target_device->op != POLY_OP_DEVICE || !multi_pm_tuple_device(ctx, multi, &n_devices)) {
    *failed = true;
    return NULL;
  }
...
static bool multi_pm_same_sharding(PolyUOp *a, PolyUOp *b) {
  ...
  for (int i = 0; i < a->arg.int_tuple.n; i++)
    if (a->arg.int_tuple.vals[i] != b->arg.int_tuple.vals[i] || a->src[i + 1] != b->src[i + 1])
      return false;
  return true;
}
...
static PolyUOp **multi_pm_shard_srcs(
  ...
  *failed = false;
  ...
  for (int i = 0; i < n_src; i++) {
    ...
    if (!sharding_range && src[i] && src[i]->op == POLY_OP_UNSHARD && src[i]->n_src > 1)
  ...
  for (int i = 0; i < n_src; i++) {
    ...
    if (poly_uop_axis(ctx, src[i], &existing_axis) && existing_axis == source_axis) {
      ...
      local[i] = full && multi_pm_is_broadcast_axis(ctx, src[i], out_shape, out_ndim, axis) ? full
                 : full && source_axis >= 0
                     ? multi_pm_shard_value(ctx, full, source_axis, sharding_range, failed)
                     : NULL;
    }
    if (!local[i] || *failed) {
      free(local);
...
static PolyUOp *multi_pm_alu(PolyCtx *ctx, PolyUOp *root, PolyUOp **src, bool *failed) {
  *failed = false;
  ...
  for (int i = 0; i < root->n_src; i++) {
    if (src[i]->op == POLY_OP_UNSHARD)
      ...
      can_handle = can_handle &&
                   (poly_uop_ndim(ctx, src[i]) == 0 || multi_pm_shape_equal(ctx, src[i], target));
  }
  PolyUOp **local = NULL;
  ...
  if (can_handle) {
    ...
    for (int i = 0; i < root->n_src; i++)
      local[i] = src[i]->op == POLY_OP_UNSHARD ? src[i]->src[0]
    ...
    local = multi_pm_shard_srcs(ctx, src, root->n_src, axis, &sharding_range, failed);
  }
  if (!local || *failed) {
    free(local);
    return NULL;
...
static PolyUOp *multi_pm_reduce(PolyCtx *ctx, PolyUOp *root, PolyUOp *multi, bool *failed) {
  *failed = false;
  ...
  bool cast_collective = poly_getenv_int("ALLREDUCE_CAST", 1) != 0 &&
                         multi->src[0]->op == POLY_OP_CAST && multi->src[0]->n_src == 1 &&
...
static bool multi_pm_shape_equal(PolyCtx *ctx, PolyUOp *a, PolyUOp *b) {
  ...
  for (int i = 0; i < a_ndim; i++)
    if (!multi_pm_expr_equal(ctx, poly_uop_shape_dim(ctx, a, i), poly_uop_shape_dim(ctx, b, i)))
      return false;
  return true;
}
...
static PolyUOp *multi_pm_shard_subview(PolyCtx *ctx, PolyUOp *full, PolyUOp *multi, bool *failed) {
  *failed = false;
  ...
  if (full->op == POLY_OP_EXPAND && full->n_src == 2 && poly_uop_ndim(ctx, full->src[0]) == 0) {
...
static PolyUOp *multi_pm_exact_divide(PolyCtx *ctx, PolyUOp *value, int64_t divisor) {
  if (!value || divisor <= 0) return NULL;
  PolyUOp *count = multi_index_const(ctx, divisor);
  PolyUOp *remainder =
      count ? poly_uop2(ctx, POLY_OP_FLOORMOD, POLY_WEAKINT, value, count, poly_arg_none()) : NULL;
  ...
  PolyUOp *quotient = poly_uop2(ctx, POLY_OP_FLOORDIV, POLY_WEAKINT, value, count, poly_arg_none());
...
static PolyUOp *multi_pm_mop_bounds(
  ...
  if (!ctx || !value || (op != POLY_OP_PAD && op != POLY_OP_SHRINK) || ndim < 0 ||
      ndim > POLY_MAX_DIMS)
    return NULL;
...
static PolyUOp *multi_pm_param(PolyCtx *ctx, PolyUOp *param, bool *failed) {
  *failed = false;
  if (!param || param->op != POLY_OP_PARAM || param->arg.kind != POLY_ARG_PARAM ||
      !param->arg.param || !param->arg.param->has_axis || !param->arg.param->device_is_tuple ||
      param->arg.param->n_devices <= 0) {
    *failed = true;
    return NULL;
  }
  ...
  for (int i = 0; i < ndim; i++) {
    ...
    if (i == axis)
      local_dims[i] = multi_pm_exact_divide(ctx, local_dims[i], param->arg.param->n_devices);
    if (!local_dims[i]) {
      *failed = true;
      return NULL;
    }
...
static bool multi_pm_sharding(
  ...
  if (!multi || multi->op != POLY_OP_UNSHARD || multi->arg.kind != POLY_ARG_INT_TUPLE ||
      multi->arg.int_tuple.n <= 0 || multi->arg.int_tuple.n > POLY_MAX_DIMS ||
...
static PolyUOp *multi_pm_movement(
  ...
  *failed = false;
  ...
  if (root->op == POLY_OP_RESHAPE && root->n_src == 2) {
    ...
    for (int i = 0; i < ndim; i++)
      dims[i] = poly_uop_shape_dim(ctx, root, i);
    prefix[0] = multi_index_const(ctx, 1);
    for (int i = 0; i < ndim; i++) {
      prefix[i + 1] =
          prefix[i] && dims[i]
              ? poly_uop2(ctx, POLY_OP_MUL, POLY_WEAKINT, prefix[i], dims[i], poly_arg_none())
              : NULL;
      prefix[i + 1] =
          prefix[i + 1] ? poly_graph_rewrite(ctx, prefix[i + 1], poly_symbolic()) : NULL;
    }
    ...
    for (int i = 0; i < n_sharding; i++) {
      ...
      for (int j = 0; target && j < axes[i]; j++) {
        ...
        target =
            dim ? poly_uop2(ctx, POLY_OP_MUL, POLY_WEAKINT, target, dim, poly_arg_none()) : NULL;
  ...
  if (root->op == POLY_OP_EXPAND && root->n_src == 2) {
    ...
    PolyUOp *local = poly_uop(ctx, POLY_OP_EXPAND, root->dtype, expand_src, 2, poly_arg_none());
    int64_t new_axes[POLY_MAX_DIMS];
    for (int i = 0; i < n_sharding; i++)
  ...
  if (root->op == POLY_OP_PAD && root->n_src == 3) {
    ...
    for (int i = 0; i < local_ndim; i++) {
      ...
      sizes[i] =
          sharded ? poly_uop_shape_dim(ctx, multi->src[0], i) : multi_pm_shape_arg_item(src[2], i);
      if (!offsets[i] || !sizes[i]) {
        *failed = true;
        return NULL;
      }
      if (sharded && (!multi_pm_expr_equal(ctx, multi_pm_shape_arg_item(src[1], i), zero) ||
                      !multi_pm_expr_equal(
                          ctx, multi_pm_shape_arg_item(src[2], i), poly_uop_shape_dim(ctx, multi, i)
                      ))) {
        *failed = true;
        return NULL;
      }
  ...
  if (root->op == POLY_OP_PERMUTE && root->n_src == 1 && root->arg.kind == POLY_ARG_INT_TUPLE) {
    int64_t new_axes[POLY_MAX_DIMS];
    for (int i = 0; i < n_sharding; i++) {
      new_axes[i] = -1;
  ...
  if (root->op == POLY_OP_SHRINK && root->n_src == 3) {
    ...
    for (int i = 0; i < n_sharding; i++) {
      ...
      PolyUOp *owned_start =
          local_axis
              ? poly_uop2(ctx, POLY_OP_MUL, POLY_WEAKINT, ranges[i], local_axis, poly_arg_none())
              : NULL;
      owned_start = owned_start ? poly_graph_rewrite(ctx, owned_start, poly_symbolic()) : NULL;
      ...
      if (multi_pm_expr_equal(ctx, start, zero) && multi_pm_expr_equal(ctx, size, public_axis)) {
        starts[axis] = zero;
        sizes[axis] = local_axis;
      ...
      PolyUOp *device = multi_pm_tuple_device(ctx, multi, NULL);
      if (n_sharding != 1 || !device || device->arg.kind != POLY_ARG_STRING_TUPLE) {
        *failed = true;
        return NULL;
      }
      for (int lane = 0; lane < counts[i]; lane++) {
        PolyUOp *partition_start =
            lane == 0 ? zero
                      : poly_uop2(
                            ctx, POLY_OP_MUL, POLY_WEAKINT, multi_index_const(ctx, lane),
                            local_axis, poly_arg_none()
                        );
        partition_start =
            partition_start ? poly_graph_rewrite(ctx, partition_start, poly_symbolic()) : NULL;
  ...
  if (root->op == POLY_OP_FLIP && root->n_src == 1 && root->arg.kind == POLY_ARG_INT_TUPLE &&
      root->arg.int_tuple.n == local_ndim) {
    for (int i = 0; i < n_sharding; i++)
...
static PolyUOp *multi_pm_stack(PolyCtx *ctx, PolyUOp *root, PolyUOp **src, bool *failed) {
  *failed = false;
  PolyUOp *target = NULL;
  ...
  for (int i = 0; i < root->n_src; i++) {
    if (src[i] && src[i]->op == POLY_OP_UNSHARD) {
      if (src[i]->arg.kind != POLY_ARG_INT_TUPLE || src[i]->arg.int_tuple.n != n_sharding ||
          src[i]->n_src != n_sharding + 1) {
        same = false;
        break;
      }
      for (int j = 0; j < n_sharding; j++)
        if (src[i]->arg.int_tuple.vals[j] != axes[j] || src[i]->src[j + 1] != ranges[j])
          same = false;
      local[i] = src[i]->src[0];
    } else {
      local[i] = src[i];
  ...
  if (!same) {
    ...
    local = multi_pm_shard_srcs(ctx, src, root->n_src, axis - 1, &sharding_range, failed);
    if (!local || *failed) {
      free(local);
      return NULL;
    }
    PolyUOp *stack = poly_uop(ctx, POLY_OP_STACK, root->dtype, local, root->n_src, root->arg);
    PolyUOp *ret = stack ? multi_pm_wrap_axis(ctx, stack, axis, sharding_range) : NULL;
    free(local);
    if (!ret) *failed = true;
    return ret;
  }
  ...
  *failed = false;
  ...
  for (int i = 0; i < n_sharding; i++) {
    ...
    if (!local || vmin < 0 || shard_max < 0 || vmax >= shard_max) {
      ...
      local = poly_uop2(ctx, POLY_OP_FLOORDIV, POLY_WEAKINT, diff, shard_size, poly_arg_none());
      local = local ? poly_graph_rewrite(ctx, local, poly_symbolic()) : NULL;
      vmin = vmax = -1;
      if (local) poly_uop_minmax(ctx, local, &vmin, &vmax);
  ...
  PolyUOp *ret = !*failed
                     ? poly_uop(ctx, POLY_OP_INDEX, root->dtype, local_src, root->n_src, root->arg)
                     : NULL;
  free(local_src);
  if (!ret) *failed = true;
  return ret;
...
static PolyUOp *multi_pm_store_dest(
  ...
  *failed = false;
  ...
  PolyUOp *ret =
      !*failed ? poly_uop(ctx, root->op, root->dtype, local, root->n_src, root->arg) : NULL;
  free(local);
  if (!ret) *failed = true;
  return ret;
}
...
static PolyUOp *multi_pm_passthrough(
  ...
  if (!ctx || !root || !src || !failed || root->n_src < 1 ||
      (wrap_multi &&
       (!src[0] || src[0]->op != POLY_OP_UNSHARD || src[0]->arg.kind != POLY_ARG_INT_TUPLE ||
  ...
  PolyUOp **local_src =
      root->n_src > 16 ? malloc((size_t)root->n_src * sizeof(*local_src)) : local_src_stack;
  if (!local_src) {
    *failed = true;
    return NULL;
  }
  ...
  PolyUOp *local = poly_uop(ctx, root->op, root->dtype, local_src, root->n_src, root->arg);
  if (local_src != local_src_stack) free(local_src);
  if (!local) {
    *failed = true;
    return NULL;
...
static PolyUOp *multi_pm_function(PolyCtx *ctx, PolyUOp *call, PolyUOp **src, bool *failed) {
  if (!ctx || !call || call->op != POLY_OP_FUNCTION || call->n_src < 1 || !call->src[0] || !src ||
      !failed) {
    *failed = true;
    return NULL;
  }
  if (call->arg.kind == POLY_ARG_CALL_INFO && call->arg.call_info &&
    ...
    return (call->tag != 0 || call->tag_arg.kind != POLY_ARG_NONE)
               ? poly_uop_tagged_arg(
                     ctx, call->op, call->dtype, src, call->n_src, call->arg, call->tag,
                     call->tag_arg
                 )
               : poly_uop(ctx, call->op, call->dtype, src, call->n_src, call->arg);
  }
  ...
  PolyUOp **call_src =
      call->n_src > 16 ? malloc((size_t)call->n_src * sizeof(*call_src)) : call_src_stack;
  if (!call_src) {
    *failed = true;
    return NULL;
  }
  ...
  for (int i = 1; i < call->n_src; i++) {
    if (src[i] && src[i]->op == POLY_OP_UNSHARD) {
      if (src[i]->arg.kind != POLY_ARG_INT_TUPLE || src[i]->n_src != src[i]->arg.int_tuple.n + 1) {
  ...
  PolyUOp *result = NULL;
  if (!has_multi_output) {
    result = multi_clone(ctx, call, call_src, call->n_src);
  } else {
    PolyUOp *body_src_stack[16], *result_src_stack[16];
    PolyUOp **body_src =
        new_body->n_src > 16 ? malloc((size_t)new_body->n_src * sizeof(*body_src)) : body_src_stack;
    ...
    for (int i = 0; i < new_body->n_src; i++)
      body_src[i] =
          new_body->src[i]->op == POLY_OP_UNSHARD ? new_body->src[i]->src[0] : new_body->src[i];
    PolyUOp *shard_body =
        poly_uop(ctx, POLY_OP_TUPLE, POLY_VOID, body_src, new_body->n_src, poly_arg_none());
    call_src[0] = shard_body;
    PolyUOp *shard_call = shard_body ? multi_clone(ctx, call, call_src, call->n_src) : NULL;
    bool ok = shard_call != NULL;
    for (int i = 0; ok && i < new_body->n_src; i++) {
      PolyUOp *selected =
          poly_uop1(ctx, POLY_OP_GETTUPLE, new_body->src[i]->dtype, shard_call, poly_arg_int(i));
    ...
    if (ok)
      result =
          poly_uop(ctx, POLY_OP_TUPLE, POLY_VOID, result_src, new_body->n_src, poly_arg_none());
...
static PolyUOp *multi_pm_copy(PolyCtx *ctx, PolyUOp *copy, PolyUOp *source, bool *failed) {
  ...
  if (copy->arg.kind == POLY_ARG_STRING_TUPLE) {
    ...
    PolyUOp *result =
        ok ? poly_uop(ctx, POLY_OP_MSTACK, copy->dtype, copies, n_devices, poly_arg_none()) : NULL;
    free(copies);
  ...
  for (;;) {
    ...
    for (int t = 0; t < n_topo; t++) {
      ...
      if (u->op == POLY_OP_PARAM && u->arg.kind == POLY_ARG_PARAM && u->arg.param &&
          u->arg.param->has_axis) {
        result = multi_pm_param(ctx, u, &rule_failed);
      } else if (u->op == POLY_OP_COPY && u->n_src == 1 && ns[0] && ns[0]->op == POLY_OP_UNSHARD) {
        PolyUOp *target = NULL;
        if (u->arg.kind == POLY_ARG_STRING)
          target = poly_device_uop_from_name(ctx, u->arg.str);
        else if (u->arg.kind == POLY_ARG_STRING_TUPLE)
          target = poly_device_uop_from_names(ctx, u->arg.string_tuple.vals, u->arg.string_tuple.n);
        result = target ? multi_pm_copy_multi(ctx, ns[0], target, &rule_failed) : NULL;
      ...
      } else if (u->op == POLY_OP_SHRINK && u->n_src == 3 && ns[0] && ns[0]->op == POLY_OP_MSTACK) {
        result = multi_pm_mstack_early_shrink(ctx, u, ns[0], ns[1], ns[2], &rule_failed);
      } else if (u->op == POLY_OP_MSELECT && u->n_src == 1 && ns[0] &&
                 poly_opset_has(POLY_GROUP_MOVEMENT, ns[0]->op) &&
                 ns[0]->n_src >= 1) {
        /* Pinned schedule/multi.py:32-34 moves selection before movement. */
        PolyUOp *selected =
            poly_uop1(ctx, POLY_OP_MSELECT, ns[0]->src[0]->dtype, ns[0]->src[0], u->arg);
        ...
        if (!selected || !movement_src) {
          ...
          for (int i = 1; i < ns[0]->n_src; i++)
            movement_src[i] = ns[0]->src[i];
          result = multi_clone(ctx, ns[0], movement_src, ns[0]->n_src);
      ...
      } else if (u->op == POLY_OP_REDUCE && u->n_src == 1 && ns[0] && ns[0]->op == POLY_OP_UNSHARD) {
        result = multi_pm_reduce(ctx, u, ns[0], &rule_failed);
      } else if ((u->op == POLY_OP_RESHAPE || u->op == POLY_OP_EXPAND || u->op == POLY_OP_PAD || u->op == POLY_OP_SHRINK || u->op == POLY_OP_PERMUTE || u->op == POLY_OP_FLIP) && u->n_src >= 1 && ns[0] && ns[0]->op == POLY_OP_UNSHARD) {
        result = multi_pm_movement(ctx, u, ns[0], ns, &rule_failed);
      } else if (u->op == POLY_OP_INDEX && u->n_src >= 2 && ns[0] && ns[0]->op == POLY_OP_UNSHARD) {
        ...
        PolyUOp *store = poly_uop2(ctx, POLY_OP_STORE, POLY_VOID, dest, value, poly_arg_none());
        PolyUOp *after =
        ...
        for (int i = 0; i < u->n_src; i++)
          if (ns[i] && ns[i]->op == POLY_OP_UNSHARD) has_multi_source = true;
        if (has_multi_source) result = multi_pm_passthrough(ctx, u, ns, false, &rule_failed);
      } else if ((u->op == POLY_OP_CAST || u->op == POLY_OP_BITCAST || u->op == POLY_OP_CONTIGUOUS || u->op == POLY_OP_DETACH || u->op == POLY_OP_CONTIGUOUS_BACKWARD) && u->n_src == 1 && ns[0] && ns[0]->op == POLY_OP_UNSHARD) {
        result = multi_pm_passthrough(ctx, u, ns, true, &rule_failed);
      } else if (u->op == POLY_OP_STORE && u->n_src >= 2 && ns[1] && ns[1]->op == POLY_OP_UNSHARD) {
      ...
      } else if (u->op == POLY_OP_STORE && u->n_src >= 1 && ns[0] && ns[0]->op == POLY_OP_UNSHARD) {
