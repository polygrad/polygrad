...
static const int g_n_import_descs = (int)(sizeof(g_import_descs) / sizeof(g_import_descs[0]));
...
const PolyImportDesc *poly_import_desc_find(const char *model_type) {
  ...
  for (int i = 0; i < g_n_import_descs; i++) {
    if (strcmp(g_import_descs[i].model_type, model_type) == 0) return &g_import_descs[i];
  }
  return NULL;
}
