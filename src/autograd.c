...
static PolyUOp *reduce_to_shape(PolyCtx *ctx, PolyUOp *grad, PolyShape target) {
  ...
  PolyShape gshape = poly_uop_shape_cached(grad);
  if (gshape.ndim < 0) {
  ...
  if (gshape.ndim > target.ndim) {
    ...
    gshape = poly_uop_shape_cached(grad);
...
static PolyUOp *ones_like(PolyCtx *ctx, PolyUOp *u) {
  ...
  PolyShape s = poly_uop_shape_cached(u);
  ...
  for (int i = n_walk - 1; i >= 0; i--) {
    ...
    switch (u->op) {
      ...
      case POLY_OP_BUFFERIZE: {
        PolyShape s0 = poly_uop_shape_cached(u->src[0]);
      ...
      case POLY_OP_SUB: {
        PolyShape s0 = poly_uop_shape_cached(u->src[0]);
        PolyShape s1 = poly_uop_shape_cached(u->src[1]);
      ...
      case POLY_OP_POW: {
        ...
        PolyShape s0 = poly_uop_shape_cached(u->src[0]);  /* base */
        PolyShape s1 = poly_uop_shape_cached(u->src[1]);  /* exponent */
        PolyUOp *g0 = cast_to(ctx, g, u->dtype);
      ...
      case POLY_OP_WHERE: {
        PolyShape s1 = poly_uop_shape_cached(u->src[1]);
        PolyShape s2 = poly_uop_shape_cached(u->src[2]);
        PolyUOp *zero = const_scalar(ctx, u->dtype, 0.0);
        PolyUOp *g0 = cast_to(ctx, g, u->dtype);
      ...
      case POLY_OP_PAD: {
        ...
        PolyShape in_shape = poly_uop_shape_cached(u->src[0]);
