/*
 * optim.c -- Open optimizer graph helpers.
 */

#include "nn/optim.h"
#include "tensor.h"
#include <stdlib.h>
#include <string.h>

static int64_t optim_uop_numel(PolyCtx *ctx, PolyUOp *u) {
  if (!ctx || !u) return -1;
  int ndim = poly_uop_ndim(ctx, u);
  if (ndim < 0) return -1;
  if (ndim == 0) return 1;
  const int64_t *dims = poly_uop_max_shape_dims(ctx, u);
  return poly_shape_numel_checked(dims, ndim);
}

static PolyUOp *optim_lr_uop(PolyCtx *ctx, PolyUOp *lr) {
  PolyDType dtype;
  if (!ctx || !lr || !poly_ctx_owns_ptr(ctx, lr)) return NULL;
  dtype = lr->dtype;
  if (!poly_dtype_is_float(dtype) || dtype.bitsize < 32) return NULL;
  int ndim = poly_uop_ndim(ctx, lr);
  if (ndim == 0) return lr;
  if (ndim != 1) return NULL;
  const int64_t *dims = poly_uop_max_shape_dims(ctx, lr);
  if (!dims || dims[0] != 1) return NULL;
  PolyUOp *dim = poly_uop_shape_dim(ctx, lr, 0);
  int64_t static_dim = 0;
  if (!dim || poly_uop_const_i64(dim, &static_dim) != 0 || static_dim != 1) return NULL;
  return lr;
}

/* Current nn/optim.py uses Python float operands. ElementwiseMixin.ufix keeps
 * them weak and UOp shape inference owns broadcasting (elementwise.py:19-34). */
static PolyUOp *optim_float(PolyCtx *ctx, double value) {
  return ctx ? poly_uop_const_typed(ctx, POLY_WEAKFLOAT, value) : NULL;
}

/* Pinned ElementwiseMixin.sub (mixin/elementwise.py:90-109) is
 * `a + (-b)`, and negation is `b * -1` (elementwise.py:57-67). */
static PolyUOp *optim_sub(PolyCtx *ctx, PolyUOp *a, PolyUOp *b) {
  return poly_uop_sub(ctx, a, b);
}

/* Pinned ElementwiseMixin.div performs `a * b.reciprocal()` after
 * broadcasting (mixin/elementwise.py:206-231). */
static PolyUOp *optim_div(PolyCtx *ctx, PolyUOp *a, PolyUOp *b) {
  return poly_uop_div(ctx, a, b);
}

static PolyUOp *optim_assign_uop_target(PolyCtx *ctx, PolyUOp *target_uop, PolyUOp *value) {
  if (!ctx || !target_uop || !value) return NULL;
  if (!poly_dtype_eq(target_uop->dtype, value->dtype)) return NULL;

  /* Build the same tinygrad effect shape as Tensor.assign without mutating the
   * target yet: AFTER(target, STORE(target, next_value)). */
  PolyUOp *store = poly_uop_store_val(ctx, target_uop, value);
  if (!store) return NULL;
  PolyUOp *src[2] = {target_uop, store};
  PolyUOp *after = poly_uop(ctx, POLY_OP_AFTER, target_uop->dtype, src, 2, poly_arg_none());
  return after;
}

static bool optim_tensor_has_logical(const PolyTensor *tensor) {
  /* Polygrad logical-lifetime boundary: a mutation target with an exact
   * portable root keeps that effect graph; physical-only targets do not gain
   * one from another target in the same optimizer batch. */
  PolyLogicalState state = poly_tensor_logical_state(tensor);
  return state == POLY_LOGICAL_AVAILABLE || state == POLY_LOGICAL_RETIRED;
}

static PolyUOp *optim_adam_bias_update(PolyCtx *ctx, PolyUOp *target, double beta) {
  /* Tinygrad 2026-08-22/a9069c177a9d LAMB._step updates each bias counter as
   * `self.b*_t *= self.b*` independently of parameter order
   * (nn/optim.py:166-168). */
  PolyUOp *scale = optim_float(ctx, beta);
  PolyUOp *value = scale ? poly_uop_mul(ctx, target, scale) : NULL;
  return value ? optim_assign_uop_target(ctx, target, value) : NULL;
}

/* nn.optim.LARS/LAMB use a scalar L2 norm and a nested positive-norm guard.
 * Keep the guards in the graph, including the zero-gradient branch. */
static PolyUOp *optim_norm(PolyCtx *ctx, PolyUOp *x) {
  if (!x) return NULL;
  int ndim = poly_uop_ndim(ctx, x);
  int64_t axes[POLY_MAX_DIMS];
  if (ndim < 0 || ndim > POLY_MAX_DIMS) return NULL;
  for (int i = 0; i < ndim; i++)
    axes[i] = i;
  PolyDType acc;
  if (!poly_sum_acc_dtype(x->dtype, &acc)) return NULL;
  PolyUOp *square = poly_uop_mul(ctx, x, x);
  PolyUOp *sum =
      poly_uop_reduce_axis(ctx, POLY_OP_ADD, poly_uop_cast(ctx, square, acc), axes, ndim);
  if (poly_dtype_is_float(x->dtype)) sum = poly_uop_cast(ctx, sum, x->dtype);
  return sum ? poly_uop_alu1(ctx, POLY_OP_SQRT, sum) : NULL;
}

static PolyUOp *optim_trust_ratio(PolyCtx *ctx, PolyUOp *r1, PolyUOp *r2, PolyUOp *ratio) {
  PolyUOp *zero = poly_uop_const_int(ctx, 0), *one = optim_float(ctx, 1.0);
  return poly_uop_where(
      ctx, poly_uop_alu2(ctx, POLY_OP_CMPLT, zero, r1),
      poly_uop_where(ctx, poly_uop_alu2(ctx, POLY_OP_CMPLT, zero, r2), ratio, one), one
  );
}

int poly_optim_build_update(
    PolyCtx *ctx,
    const PolyOptimConfig *cfg,
    PolyUOp *lr,
    PolyUOp *param,
    PolyUOp *grad,
    PolyUOp *m_buf,
    PolyUOp *v_buf,
    PolyUOp *bc1_buf,
    PolyUOp *bc2_buf,
    int64_t numel,
    PolyOptimUpdate *out
) {
  if (!ctx || !cfg || !param || !grad || !out || numel <= 0) return -1;
  memset(out, 0, sizeof(*out));

  PolyUOp *param_value = param;
  PolyUOp *grad_value = grad;
  PolyUOp *lr_value = optim_lr_uop(ctx, lr);
  if (!grad_value || !lr_value) return -1;

  switch (cfg->kind) {
  case POLY_OPTIM_SGD:
  case POLY_OPTIM_LARS: {
    PolyUOp *base = poly_uop_detach(ctx, param_value);
    PolyUOp *g = grad_value;
    if (!base) return -1;
    bool pre_wd = cfg->kind == POLY_OPTIM_SGD || cfg->pre_wd;
    PolyUOp *r = optim_float(ctx, 1.0);
    if (cfg->kind == POLY_OPTIM_LARS && cfg->tcoef != 0) {
      PolyUOp *r1 = optim_norm(ctx, base), *r2 = optim_norm(ctx, g);
      PolyUOp *ratio = poly_uop_div(
          ctx, poly_uop_mul(ctx, optim_float(ctx, cfg->tcoef), r1),
          poly_uop_add(ctx, r2, poly_uop_mul(ctx, optim_float(ctx, cfg->weight_decay), r1))
      );
      r = optim_trust_ratio(ctx, r1, r2, ratio);
    }
    if (pre_wd && cfg->weight_decay > 0.0f) {
      PolyUOp *wd = optim_float(ctx, cfg->weight_decay);
      g = poly_uop_add(ctx, g, poly_uop_mul(ctx, wd, base));
    }
    if (cfg->classic) {
      g = r ? poly_uop_mul(ctx, g, r) : NULL;
      g = g ? poly_uop_mul(ctx, g, lr_value) : NULL;
    }
    if (cfg->momentum > 0.0f) {
      if (!m_buf) return -1;
      PolyUOp *m_value_root = m_buf;
      PolyUOp *mom = optim_float(ctx, cfg->momentum);
      if (!m_value_root || !mom) return -1;
      PolyUOp *m_value = poly_uop_add(ctx, poly_uop_mul(ctx, mom, m_value_root), g);
      out->m_new = optim_assign_uop_target(ctx, m_buf, m_value);
      PolyUOp *m_current = out->m_new;
      if (!m_value || !out->m_new || !m_current) return -1;
      g = cfg->nesterov ? poly_uop_add(ctx, g, poly_uop_mul(ctx, mom, m_current)) : m_current;
    }
    if (cfg->kind == POLY_OPTIM_LARS && cfg->n_ns_coefficients) {
      int ndim = poly_uop_ndim(ctx, g);
      const int64_t *dims = poly_uop_max_shape_dims(ctx, g);
      if (ndim < 1 || !dims || dims[0] <= 0) return -1;
      int64_t original[POLY_MAX_DIMS];
      memcpy(original, dims, (size_t)ndim * sizeof(int64_t));
      int64_t matrix[] = {dims[0], numel / dims[0]};
      g = poly_uop_newton_schulz(
          ctx, poly_uop_reshape(ctx, g, matrix, 2), cfg->ns_steps, cfg->ns_coefficients,
          cfg->n_ns_coefficients, 1e-7
      );
      g = g ? poly_uop_reshape(ctx, g, original, ndim) : NULL;
    }
    if (!cfg->classic) {
      g = r ? poly_uop_mul(ctx, g, r) : NULL;
      g = g ? poly_uop_mul(ctx, g, lr_value) : NULL;
    }
    if (!pre_wd && cfg->weight_decay > 0)
      g = poly_uop_add(
          ctx, g,
          poly_uop_mul(ctx, poly_uop_mul(ctx, optim_float(ctx, cfg->weight_decay), lr_value), base)
      );
    out->param_new = optim_sub(ctx, base, g ? poly_uop_cast(ctx, g, param->dtype) : NULL);
    return (out->param_new && (cfg->momentum <= 0.0f || out->m_new)) ? 0 : -1;
  }
  case POLY_OPTIM_ADAM:
  case POLY_OPTIM_LAMB:
  case POLY_OPTIM_ADAMW: {
    if (!m_buf || !v_buf || !bc1_buf || !bc2_buf) return -1;
    PolyUOp *m_value_root = m_buf;
    PolyUOp *v_value_root = v_buf;
    if (!m_value_root || !v_value_root) return -1;

    PolyUOp *b1 = optim_float(ctx, cfg->beta1);
    PolyUOp *b2 = optim_float(ctx, cfg->beta2);
    PolyUOp *one_minus_b1 = optim_float(ctx, 1.0 - cfg->beta1);
    PolyUOp *one_minus_b2 = optim_float(ctx, 1.0 - cfg->beta2);
    PolyUOp *bc1_scale = optim_float(ctx, cfg->beta1);
    PolyUOp *bc2_scale = optim_float(ctx, cfg->beta2);
    if (!b1 || !b2 || !one_minus_b1 || !one_minus_b2 || !bc1_scale || !bc2_scale) return -1;

    PolyUOp *m_value = poly_uop_add(
        ctx, poly_uop_mul(ctx, b1, m_value_root), poly_uop_mul(ctx, one_minus_b1, grad_value)
    );
    PolyUOp *g_sq = poly_uop_mul(ctx, grad_value, grad_value);
    PolyUOp *v_value = poly_uop_add(
        ctx, poly_uop_mul(ctx, b2, v_value_root), poly_uop_mul(ctx, one_minus_b2, g_sq)
    );

    PolyUOp *bc1_value = poly_uop_mul(ctx, bc1_buf, bc1_scale);
    PolyUOp *bc2_value = poly_uop_mul(ctx, bc2_buf, bc2_scale);
    out->m_new = optim_assign_uop_target(ctx, m_buf, poly_uop_cast(ctx, m_value, m_buf->dtype));
    out->v_new = optim_assign_uop_target(ctx, v_buf, poly_uop_cast(ctx, v_value, v_buf->dtype));
    out->bc1_new = optim_assign_uop_target(ctx, bc1_buf, bc1_value);
    out->bc2_new = optim_assign_uop_target(ctx, bc2_buf, bc2_value);
    PolyUOp *m_current = out->m_new;
    PolyUOp *v_current = out->v_new;
    if (!m_value || !v_value || !bc1_value || !bc2_value || !out->m_new || !out->v_new ||
        !out->bc1_new || !out->bc2_new || !m_current || !v_current)
      return -1;

    PolyUOp *one_bc1 = optim_float(ctx, 1.0);
    PolyUOp *one_bc2 = optim_float(ctx, 1.0);
    PolyUOp *bc1_denom = optim_sub(ctx, one_bc1, out->bc1_new);
    PolyUOp *bc2_denom = optim_sub(ctx, one_bc2, out->bc2_new);
    PolyUOp *m_hat = optim_div(ctx, m_current, bc1_denom);
    PolyUOp *v_hat = optim_div(ctx, v_current, bc2_denom);
    PolyUOp *eps = optim_float(ctx, cfg->eps);
    PolyUOp *denom =
        (v_hat && eps) ? poly_uop_add(ctx, poly_uop_alu1(ctx, POLY_OP_SQRT, v_hat), eps) : NULL;
    PolyUOp *up = denom ? optim_div(ctx, m_hat, denom) : NULL;
    PolyUOp *base = poly_uop_detach(ctx, param_value);
    PolyUOp *wd = optim_float(ctx, cfg->weight_decay);
    up = (up && wd) ? poly_uop_add(ctx, up, poly_uop_mul(ctx, wd, base)) : NULL;
    /* Current LAMB returns `self.lr * r * up` and leaves `[1]`/parameter
     * broadcasting to UOp shape inference (nn/optim.py:168-178). */
    PolyUOp *r = optim_float(ctx, 1.0);
    if (cfg->kind == POLY_OPTIM_LAMB) {
      PolyUOp *r1 = optim_norm(ctx, base), *r2 = optim_norm(ctx, up);
      r = optim_trust_ratio(ctx, r1, r2, poly_uop_div(ctx, r1, r2));
    }
    PolyUOp *lr_scaled = r ? poly_uop_mul(ctx, lr_value, r) : NULL;
    PolyUOp *step = lr_scaled ? poly_uop_mul(ctx, lr_scaled, up) : NULL;
    out->param_new = optim_sub(ctx, base, step ? poly_uop_cast(ctx, step, param->dtype) : NULL);
    return (out->param_new && out->m_new && out->v_new && out->bc1_new && out->bc2_new) ? 0 : -1;
  }
  default:
    return -1;
  }
}

int poly_optim_build_step(
    PolyCtx *ctx,
    const PolyOptimConfig *cfg,
    PolyTensor *lr,
    PolyTensor **params,
    PolyTensor **grads,
    int n_params,
    PolyTensor **m_tensors,
    PolyTensor **v_tensors,
    PolyTensor *bc1_tensor,
    PolyTensor *bc2_tensor,
    PolyTensor **out_tensors,
    int out_cap
) {
  if (!ctx || !cfg || !lr || !params || !grads || n_params <= 0) return -1;
  bool adam =
      (cfg->kind == POLY_OPTIM_ADAM || cfg->kind == POLY_OPTIM_ADAMW || cfg->kind == POLY_OPTIM_LAMB
      );
  bool sgd_momentum =
      ((cfg->kind == POLY_OPTIM_SGD || cfg->kind == POLY_OPTIM_LARS) && cfg->momentum > 0.0f);
  PolyUOp *lr_physical = optim_lr_uop(ctx, poly_tensor_uop_physical(lr));
  PolyUOp *lr_logical = optim_lr_uop(ctx, poly_tensor_uop_logical(lr));
  if (!lr_physical) return -1;
  int needed = n_params;
  if (adam)
    needed += 2 * n_params + 2;
  else if (sgd_momentum)
    needed += n_params;
  if (!out_tensors || out_cap < needed) return needed;

  if ((adam || sgd_momentum) && !m_tensors) return -1;
  if (adam && (!v_tensors || !bc1_tensor || !bc2_tensor)) return -1;

  PolyUOp *bc1_logical = adam ? poly_tensor_uop_logical(bc1_tensor) : NULL;
  PolyUOp *bc2_logical = adam ? poly_tensor_uop_logical(bc2_tensor) : NULL;
  PolyUOp *bc1_physical = adam ? poly_tensor_uop_physical(bc1_tensor) : NULL;
  PolyUOp *bc2_physical = adam ? poly_tensor_uop_physical(bc2_tensor) : NULL;
  if (adam && (!bc1_physical || !bc2_physical || !poly_ctx_owns_ptr(ctx, bc1_physical) ||
               !poly_ctx_owns_ptr(ctx, bc2_physical) || optim_uop_numel(ctx, bc1_physical) != 1 ||
               optim_uop_numel(ctx, bc2_physical) != 1 ||
               poly_tensor_device(bc1_tensor) != poly_tensor_device(lr) ||
               poly_tensor_device(bc2_tensor) != poly_tensor_device(lr)))
    return -1;

  /* Validate the complete batch before constructing any assignment effects. */
  for (int i = 0; i < n_params; i++) {
    if (!params[i] || !grads[i] || poly_tensor_device(lr) != poly_tensor_device(params[i]))
      return -1;
    PolyUOp *param_physical = poly_tensor_uop_physical(params[i]);
    PolyUOp *grad_physical = poly_tensor_uop_physical(grads[i]);
    if (!param_physical || !grad_physical || !poly_ctx_owns_ptr(ctx, param_physical) ||
        !poly_ctx_owns_ptr(ctx, grad_physical))
      return -1;
    int64_t numel = optim_uop_numel(ctx, param_physical);
    if (numel <= 0 || optim_uop_numel(ctx, grad_physical) != numel) return -1;

    if (adam || sgd_momentum) {
      if (!m_tensors[i] || poly_tensor_device(m_tensors[i]) != poly_tensor_device(params[i]))
        return -1;
      PolyUOp *m_physical = poly_tensor_uop_physical(m_tensors[i]);
      if (!m_physical || !poly_ctx_owns_ptr(ctx, m_physical) ||
          optim_uop_numel(ctx, m_physical) != numel)
        return -1;
    }
    if (adam) {
      if (!v_tensors[i] || poly_tensor_device(v_tensors[i]) != poly_tensor_device(params[i]))
        return -1;
      PolyUOp *v_physical = poly_tensor_uop_physical(v_tensors[i]);
      if (!v_physical || !poly_ctx_owns_ptr(ctx, v_physical) ||
          optim_uop_numel(ctx, v_physical) != numel)
        return -1;
    }
  }

  PolyTensor **targets = calloc((size_t)needed, sizeof(PolyTensor *));
  PolyUOp **logical_effects = calloc((size_t)needed, sizeof(PolyUOp *));
  PolyUOp **physical_effects = calloc((size_t)needed, sizeof(PolyUOp *));
  if (!targets || !logical_effects || !physical_effects) {
    free(targets);
    free(logical_effects);
    free(physical_effects);
    return -1;
  }

  int rc = -1;
  int m_base = -1;
  int v_base = -1;
  int param_base = 0;
  if (adam) {
    m_base = 2;
    v_base = 2 + n_params;
    param_base = 2 + 2 * n_params;
  } else if (sgd_momentum) {
    m_base = 0;
    param_base = n_params;
  }

  for (int i = 0; i < n_params; i++) {
    PolyUOp *param_logical = poly_tensor_uop_logical(params[i]);
    PolyUOp *grad_logical = poly_tensor_uop_logical(grads[i]);
    PolyUOp *param_physical = poly_tensor_uop_physical(params[i]);
    PolyUOp *grad_physical = poly_tensor_uop_physical(grads[i]);
    bool param_has_logical = optim_tensor_has_logical(params[i]);
    bool m_has_logical = (adam || sgd_momentum) && optim_tensor_has_logical(m_tensors[i]);
    bool v_has_logical = adam && optim_tensor_has_logical(v_tensors[i]);
    bool build_logical = param_has_logical || m_has_logical || v_has_logical;

    int64_t numel = optim_uop_numel(ctx, param_physical);
    PolyOptimUpdate logical_update = {0}, physical_update = {0};
    if (build_logical &&
        (!lr_logical || !param_logical || !grad_logical || !poly_ctx_owns_ptr(ctx, lr_logical) ||
         !poly_ctx_owns_ptr(ctx, param_logical) || !poly_ctx_owns_ptr(ctx, grad_logical) ||
         optim_uop_numel(ctx, param_logical) != numel ||
         optim_uop_numel(ctx, grad_logical) != numel ||
         ((adam || sgd_momentum) &&
          (!poly_tensor_uop_logical(m_tensors[i]) ||
           !poly_ctx_owns_ptr(ctx, poly_tensor_uop_logical(m_tensors[i])) ||
           optim_uop_numel(ctx, poly_tensor_uop_logical(m_tensors[i])) != numel)) ||
         (adam && (!poly_tensor_uop_logical(v_tensors[i]) ||
                   !poly_ctx_owns_ptr(ctx, poly_tensor_uop_logical(v_tensors[i])) ||
                   optim_uop_numel(ctx, poly_tensor_uop_logical(v_tensors[i])) != numel ||
                   !bc1_logical || !bc2_logical || !poly_ctx_owns_ptr(ctx, bc1_logical) ||
                   !poly_ctx_owns_ptr(ctx, bc2_logical)))))
      goto done;
    if (poly_optim_build_update(
            ctx, cfg, lr_physical, param_physical, grad_physical,
            (adam || sgd_momentum) ? poly_tensor_uop_physical(m_tensors[i]) : NULL,
            adam ? poly_tensor_uop_physical(v_tensors[i]) : NULL, bc1_physical, bc2_physical, numel,
            &physical_update
        ) != 0 ||
        (build_logical && poly_optim_build_update(
                              ctx, cfg, lr_logical, param_logical, grad_logical,
                              (adam || sgd_momentum) ? poly_tensor_uop_logical(m_tensors[i]) : NULL,
                              adam ? poly_tensor_uop_logical(v_tensors[i]) : NULL, bc1_logical,
                              bc2_logical, numel, &logical_update
                          ) != 0))
      goto done;

    PolyUOp *param_new_physical = physical_update.param_new;
    PolyUOp *param_effect_physical =
        optim_assign_uop_target(ctx, param_physical, param_new_physical);
    PolyUOp *param_new_logical = build_logical ? logical_update.param_new : NULL;
    PolyUOp *param_effect_logical =
        build_logical ? optim_assign_uop_target(ctx, param_logical, param_new_logical) : NULL;
    if (!param_new_physical || !param_effect_physical ||
        (build_logical && (!param_new_logical || !param_effect_logical)))
      goto done;
    targets[param_base + i] = params[i];
    if (param_has_logical) logical_effects[param_base + i] = param_effect_logical;
    physical_effects[param_base + i] = param_effect_physical;

    if (sgd_momentum) {
      if (!physical_update.m_new || (build_logical && !logical_update.m_new)) goto done;
      targets[m_base + i] = m_tensors[i];
      if (m_has_logical) logical_effects[m_base + i] = logical_update.m_new;
      physical_effects[m_base + i] = physical_update.m_new;
    }

    if (adam) {
      if (!physical_update.m_new || !physical_update.v_new || !physical_update.bc1_new ||
          !physical_update.bc2_new ||
          (build_logical && (!logical_update.m_new || !logical_update.v_new ||
                             !logical_update.bc1_new || !logical_update.bc2_new)))
        goto done;
      if (i == 0) {
        targets[0] = bc1_tensor;
        physical_effects[0] = physical_update.bc1_new;
        targets[1] = bc2_tensor;
        physical_effects[1] = physical_update.bc2_new;
      } else if (physical_effects[0] != physical_update.bc1_new ||
                 physical_effects[1] != physical_update.bc2_new) {
        goto done;
      }
      targets[m_base + i] = m_tensors[i];
      if (m_has_logical) logical_effects[m_base + i] = logical_update.m_new;
      physical_effects[m_base + i] = physical_update.m_new;
      targets[v_base + i] = v_tensors[i];
      if (v_has_logical) logical_effects[v_base + i] = logical_update.v_new;
      physical_effects[v_base + i] = physical_update.v_new;
    }
  }

  if (adam) {
    if (optim_tensor_has_logical(bc1_tensor))
      logical_effects[0] = optim_adam_bias_update(ctx, bc1_logical, cfg->beta1);
    if (optim_tensor_has_logical(bc2_tensor))
      logical_effects[1] = optim_adam_bias_update(ctx, bc2_logical, cfg->beta2);
  }

  for (int i = 0; i < needed; i++)
    if (!targets[i] || !physical_effects[i] ||
        (optim_tensor_has_logical(targets[i]) && !logical_effects[i]))
      goto done;

  /* Pinned Optimizer.schedule_step advances every current Tensor.uop with
   * assign (nn/optim.py:41-57; tensor.py:230-257). Preserve that exact program
   * on the executable side, and apply it independently to the retained
   * logical operands so export/re-placement never inherits physical storage. */
  for (int i = 0; i < needed; i++) {
    PolyUOp *logical_effect = optim_tensor_has_logical(targets[i]) ? logical_effects[i] : NULL;
    if (!poly_tensor_assign_after(ctx, targets[i], logical_effect, physical_effects[i])) goto done;
  }
  memcpy(out_tensors, targets, (size_t)needed * sizeof(PolyTensor *));
  rc = needed;

done:
  free(logical_effects);
  free(physical_effects);
  free(targets);
  return rc;
}
